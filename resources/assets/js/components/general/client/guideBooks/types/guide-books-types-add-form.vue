            ...
            <div class="row">
                <div class="col-md-12">
                    <div v-if="showTitle" class="page-header has-border-bottom mb-4">
                        <div class="row"><div class="col-6"><a href="/client/v2/guide-books-types" class="text-muted d-block text-md mb-1"><i class="fas fa-arrow-circle-left"></i>&nbsp;<span class="hidden-xs">Back to </span> types list</a></div> </div>
                        <h1 class="page-title">Add Guide Books Type</h1>
                    </div>
                    <div class="page-body">
    ...
    export default {
        ...
        computed: {
            ...mapState({
                ...
                formData: (state) => {
                    return state.general.guideBookTypes.formData;
                }
            })
        },
        data:function (){
        ...
        methods:{
            ...
            addNew: async  function(){
                this.resetErrors();
                let result=await this.$store.dispatch('general/addGuideBookType',this.formData);
                if(Object.keys(result).length > 0){
                    this.setErrors(result);
                }else if(result){
                    window.location.href='guide-books-types';
                }else if(!result){
                    this.$emit('updated');
                }
            },
            loadOldData:function () {
                this.$store.dispatch('general/loadGuideBookTypeOldData',this.serve_id);
            },
