@component('mail::layout')
    ...
    Refund details
        ...
        Guest name :  {{ucfirst($data['guest_name'])}}
        Check-in date : {{$data['checkin']}}
        Check-out date : {{$data['checkout']}}
        Refunded amount : {{$data['amount']}}
        Refund Policy Applied: 7 days before check-in.


        For further details kindly Visit


@component('mail::button', ['url' => $data['url2'], 'color' => 'green' ])
    VIEW DETAILS
@endcomponent
...
@php echo $data['url2'] @endphp


    Regards,
    {{(!isset($data['companyName']) ?  config('app.name') : $data['companyName']) }} Team
