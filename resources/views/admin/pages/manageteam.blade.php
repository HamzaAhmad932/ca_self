...
<div class="m-grid__item m-grid__item--fluid m-wrapper">
    ...
    <div class="m-subheader ">
        <div class="d-flex align-items-center">
            <div class="mr-auto">
                <h3 class="m-subheader__title m-subheader__title--separator">{{__('admin/adminteamcontent.team_members')}}</h3>
                ...
                <div class="m-portlet__head-caption">
                    <div class="m-portlet__head-title">
                        <h3 class="m-portlet__head-text">
                            {{__('admin/adminteamcontent.member_list')}}
                        </h3>
                    </div>
                </div>
                        ...
                        <li class="m-portlet__nav-item">
                            <a href="#" class="btn btn-primary m-btn m-btn--pill m-btn--custom m-btn--icon m-btn--air" data-toggle="modal" data-target="#m_modal_2">
                                <span>
                                    ...
                                    <span>{{__('admin/adminteamcontent.new_member')}}</span>
                                </span>
                            </a>
                        </li>
                        ...
                        <table class="table table-striped- table-bordered table-hover table-checkable dataTable no-footer dtr-inline" id="m_table_1" role="grid" aria-describedby="m_table_1_info" style="width: 1149px;">
                            <thead>
                                <tr role="row">
                                    ...
                                    <th class="sorting" tabindex="0" aria-controls="m_table_1" rowspan="1" colspan="1" style="width: 53.45px;" aria-label="Designation: activate to sort column ascending">{{__('admin/adminteamcontent.designation')}}</th>
                                ...
                                <tbody>
                                @if(count($data['teamList']) > 0 )
                                    @foreach($data['teamList'] as $member )
                                    <tr role="row" class="odd" >
                                        <td tabindex="0" class="sorting_1">
                                            <div class="m-card-user m-card-user--sm">
                                                <div class="m-card-user__pic">
                                                    <div class="m-card-user__no-photo m--bg-fill-info"><span>{{ $member->name[0] }}</span></div>
                                                </div>
                                                <div class="m-card-user__details">
                                                    <span class="m-card-user__name">{{ $member->name }}</span>
                                                    <a href="" class="m-card-user__email m-link">{{ $member->name }}</a>
                                                </div>
                                            </div></td>
                                            <td><a class="m-link" href="mailto:{{ $member->email }}">{{ $member->email }}</a></td>
                                            <td>{{ $member->phone }}</td>
                                            <td><span class="m-badge  m-badge--metal m-badge--wide">{{ $member->hasPermissionTo('edit') }} </span></td>
                                            <td nowrap=""><span class="dropdown">
                                                ...
                                                <div class="dropdown-menu dropdown-menu-right" >
                                                   ...
                                                   {{--@if($member->status == config('db_const.user.status.active.value'))--}}

{{--<button class="MemStatusBtn dropdown-item" data-id="{{ $member->id }}" data-status="{{ config('db_const.user.status.deactive.value') }}" data-token="{{ csrf_token() }}" > <i class="la la-toggle-off"></i> {{ config('db_const.user.status.deactive.label') }} </button>--}}

                                                   {{--@elseif ($member->status == config('db_const.user.status.deactive.value') )--}}

  {{--<button class="MemStatusBtn dropdown-item" data-id="{{ $member->id }}" data-status="{{ config('db_const.user.status.active.value') }}" data-token="{{ csrf_token() }}" > <i class="la la-toggle-on"></i>{{ config('db_const.user.status.active.label') }}  </button>--}}
    ...
    <button  class="myid dropdown-item" data-id="{{ $member->id }}" data-status="{{ config('db_const.user.status.deactive.value') }}" data-token="{{ csrf_token() }}" @click.prevent="MemStatusbtn($event)" > <i class="la la-toggle-on"></i> {{ config('db_const.user.status.active.label') }} </button>

    @elseif ($member->status == config('db_const.user.status.deactive.value') )

    <button class="myid dropdown-item" data-id="{{ $member->id }}" data-status="{{ config('db_const.user.status.active.value') }}" data-token="{{ csrf_token() }}" @click.prevent="MemStatusbtn($event)" > <i class="la la-toggle-off"></i>{{ config('db_const.user.status.deactive.label') }}  </button>

    @endif

      {{--<button id="myid" class="dropdown-item" :data-id="{{ $member->id }}"--}}
              {{--data-status="{{ config('db_const.user.status.deactive.value') }}"--}}
              {{--@click.prevent="MemStatusbtn()" >--}}
          {{--<i class="la la-toggle-off"></i>--}}
          {{--{{ config('db_const.user.status.deactive.label') }} </button>--}}





 <button class="MemDelBtn dropdown-item" data-id="{{ $member->id }}" data-token="{{ csrf_token() }}" @click.prevent="MemStatusbtn($event)" ><i class="la la-trash"></i>Delete</button>
                                             </div>
                                            </span>
                                                <div id="spi_{{ $member->id }}"  class="m-loader m-loader--danger" style="width: 30px; display: none; position: absolute;"></div>
                                            <a href="memberprofile/{{ $member->id }}" class="m-portlet__nav-link btn m-btn m-btn--hover-brand m-btn--icon m-btn--icon-only m-btn--pill" title="View">
                        ...
                        <div class="row">
                            <div class="col-sm-12 col-md-5">
                                <div class="dataTables_info" id="m_table_1_info" role="status" aria-live="polite">Showing {{ count($data['teamList']) }}</div>
                            </div>
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLongTitle">{{__('admin/adminteamcontent.create_member')}}</h5>
               ...
               <form class="m-form m-form--fit" method="post" id="memForm" >
                        ...
                        <div class="m-portlet__body">
                            <div class="m-form__section m-form__section--first">
                                ...
                                <div class="form-group m-form__group" :class="{'has-error' : hasErrors.username}">
                                    <label>Username:</label>
                                    <input type="text" name="username" class="form-control m-input"  v-model="registerData.username"   >

                                    <span v-if="hasErrors.username" class="invalid-feedback" role="alert">
                                ...
                                <div class="m-form__group form-group">
                                    ...
                                    <div class="">
                                        <select id="company" class="form-control m-bootstrap-select m_selectpicker" multiple data-actions-box="true" name="company[]" v-model="registerData.company">
                                            @foreach($data['company'] as $p)
                                            <option data-content="<span class='m-badge m-badge--warning m-badge--wide m-badge--rounded'>{{ $p->name }}</span>" value="{{ $p->id }}" >{{ $p->name }}</option>
                                            @endforeach
                                        </select>
                                    </div>
            ...
            <script type="application/javascript">
                ...
                methods:{
                    registerMem(){
                        ...
                        axios.post('adminmember', _this.registerData)
