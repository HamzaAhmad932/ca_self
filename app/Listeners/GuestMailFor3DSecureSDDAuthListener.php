...
class GuestMailFor3DSecureSDDAuthListener extends GuestMailFor3DSecureAbstractListener {
    /**
    ...
    public function handle(GuestMailFor3DSecureEvent $event) {
        ...
        if($event->mailFor3DS !== GuestMailFor3DSecureEvent::MAIL_FOR_3DS_AUTH_SSD)
            return;
        ...
        $message = 'Please complete 3D Secure authentication for authorization of ' . $currency . $amount . ' for Security Damage deposit against your booking ' . $event->bookingInfo->pms_booking_id . ' ' . date('M d', strtotime($event->bookingInfo->check_in_date)). ' - ' .date('M d, Y', strtotime($event->bookingInfo->check_out_date)) .'. You can proceed by clicking on below button. ';

        if(empty($event->transaction->authenticationUrl)) {
            $buttonText = 'Authorize ' . $currency . $amount;
            $event->transaction->authenticationUrl = URL::signedRoute('checkout', ['id'=>$event->creditCardAuthorization->id, 'type'=>GuestMailFor3DSecureEvent::MAIL_FOR_3DS_AUTH_SSD]);
        }
